...
function saveOrder() {
    ...
    // Отправляем данные на сервер для обновления заказа ( отправляем в виде FormData а получаем обновленный json файл)
    fetch(`https://edu.std-900.ist.mospolytech.ru/labs/api/orders/${orderId}?api_key=9fbacd36-e75b-4d34-839b-5d03ae7386eb`, {
        method: 'PUT',
...
function confirmDelete() {
    ...
    // Отправляем запрос на удаление и также получаем обратнод данные в формате json
    fetch(`https://edu.std-900.ist.mospolytech.ru/labs/api/orders/${orderIdToDelete}?api_key=9fbacd36-e75b-4d34-839b-5d03ae7386eb`, {
        method: 'DELETE'
    })
        .then(response => response.json())
        .then((data) => {
